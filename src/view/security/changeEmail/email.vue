    ...
    <div class="box security two" v-if="step==2">
      <div class="box-body">
        <el-form :label-position="labelPosition" size="small" :model="submitForm" ref="submitForm" label-width="120px" :rules="rules">
          <el-form-item label="新邮箱地址" prop="newpassword">
    ...
    methods: {
      next () {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            this.form.taget = this.user.Phone
        ...
        this.$refs['submitForm'].validate((valid) => {
          if (valid) {
            ...
            this.$store.dispatch('changePhoneOrEmailStep2', this.submitForm).then((response) => {
              if (response) {
                this.step = 4
                this.$store.commit('changeEmail', this.submitForm.taget)
              } else {
                this.step = 1
      ...
      doVerify (n) {
        ...
        n === 0 ? this.$store.dispatch('sendsms', {'taget': this.user.Phone, smstype: '3'}) : this.$store.dispatch('sendsms', {'taget': this.submitForm.taget, smstype: '3'})
