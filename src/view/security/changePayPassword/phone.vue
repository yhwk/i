<template>
  <div class="pane-body">
    <h4 class="title">修改支付密码</h4>
    <el-steps :space="200" :active="step-1" finish-status="success" align-center>
      <el-step title="验证身份"></el-step>
      <el-step title="修改支付密码"></el-step>
      <el-step title="修改成功"></el-step>
    </el-steps>
    <div class="box security one" v-if="step==1">
      <div class="box-title">
        <img src="../../../assets/info.png">您正在通过验证短信修改 |
        <router-link to="/security/paypassword/email"  v-if="user.Email">通过已验证邮箱修改</router-link>
      </div>
      <div class="box-body">
        <div class="phone">
    ...
    <div class="box security two" v-if="step==2">
      <div class="box-body">
        <el-form :label-position="labelPosition" size="small" :model="submitForm" ref="submitForm" label-width="120px" :rules="rules">
          <el-form-item label="旧支付密码" prop="newpassword">
      ...
      submit () {
        this.$refs['submitForm'].validate((valid) => {
          if (valid) {
            ...
            this.submitForm.oldpassword = this.user.PayPassword
            this.$store.dispatch('setPayPassword', this.submitForm).then((response) => {
              if (response) {
                this.step = 4
              } else {
                this.step = 1
